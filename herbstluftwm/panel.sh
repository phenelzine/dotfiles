...
wifi=$(curl -Is https://www.google.com | head -n 1)
kernel=$(uname -r)
updates=$(apt-get --just-print upgrade | grep "^Inst.*-security.*" | cut -d ' ' -f 2 | wc -l)
uptime=$(uptime -p)
...

{
    ...
    while true ; do
        ...
        echo -n "$separator"
        echo -n "^bg()^fg(#ff6e67) ${windowtitle//^/^^} "
	
	echo -n
	echo -n "$separator^bg()"
	echo -n
	
	echo -n "^fg(#f4f99d)^bg() Wifi: ^bg()^fg()$wifi "
	echo -n
	echo -n "$separator^bg()"
	echo -n

	echo -n "^fg(#9aedfe)^bg() Kernel: ^bg()^fg()$kernel "
	echo -n "$separator^bg()"
	echo -n

	echo -n "^fg(#5af78e)^bg() Updates: ^bg()^fg()$updates "
